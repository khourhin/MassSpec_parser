...
class CompareMSForm(forms.Form):
    ...
    ms_in = forms.ModelMultipleChoiceField(
        label='MS infiles (.csv)',
        queryset=FileUpload.objects.filter(doctype='i'),
        widget=forms.CheckboxSelectMultiple,
        required=True)

    ms_bck = forms.ModelMultipleChoiceField(
        label='MS background (.csv)',
        queryset=FileUpload.objects.filter(doctype='b'),
        widget=forms.CheckboxSelectMultiple,
        required=False)

    blast_db = forms.ModelMultipleChoiceField(
        label='blast db',
